...
describe('trusted capture correlation',()=>{
 ...
 it('requires the authenticated SIP credential owner to equal the extension owner',async()=>{db.query.mockResolvedValue({rows:[]});await bindAuthenticatedOutbound({'Unique-ID':id,variable_sip_auth_username:'3001',variable_sip_auth_realm:'test'},'test');const sql=String(db.query.mock.calls[0][0]);expect(sql).toContain('e.user_id=s.user_id');expect(sql).toContain('ue.user_id=s.user_id');expect(sql).toContain('s.user_id IS NOT NULL');});
 ...
 it('incoming correlation persists stable call-time owner and native wake ID, not display number authority',async()=>{db.query.mockResolvedValueOnce({rows:[{tenant_id:10,extension_id:11,user_id:2,wake_uuid:id}]}).mockResolvedValueOnce({rows:[{channel_uuid:id}]}).mockResolvedValueOnce({rows:[]});expect(await bindIncomingChannel(id,'exact','caller@example.com')).toBe(true);expect(db.query.mock.calls[1][1]).toEqual([id,10,11,2,'exact','Unknown']);expect(db.query.mock.calls[2][1]).toEqual([id,10,11,`native-wake:${id}`,'Unknown']);});
 it('binds one exact observed outbound A-leg and persists its native history ID atomically',async()=>{
  db.query.mockResolvedValueOnce({rows:[{id:11,tenant_id:10,user_id:2}]}).mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[{channel_uuid:id}]}).mockResolvedValueOnce({rows:[{call_uuid:id}]});
  expect(await bindObservedOutboundChannel(observed())).toBe(true);
  expect(db.query.mock.calls[5][1]).toEqual([id,10,11,2,'exact@sip','+66800000000']);
 ...
 it('is idempotent only for the same channel, SIP identity, owner and native ID',async()=>{
  const route={tenant_id:10,extension_id:11,owner_user_id:2,sip_call_id:'exact@sip',direction:'outbound'};
  const cloud={call_uuid:id,tenant_id:10,extension_id:11,native_history_id:`native-outbound:${nonce}`,direction:'outbound'};
  db.query.mockResolvedValueOnce({rows:[{id:11,tenant_id:10,user_id:2}]}).mockResolvedValueOnce({rows:[]}).mockResolvedValueOnce({rows:[route]}).mockResolvedValueOnce({rows:[cloud]}).mockResolvedValueOnce({rows:[cloud]});
