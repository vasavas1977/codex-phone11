#!/usr/bin/env python3
"""Pin and loader checks for the Team Chat Mentions empty-state static release entry point."""
...
WRAPPER = ROOT / "scripts/phone11-chat-mentions-empty-static-rollout.py"
CONTROLLER = ROOT / "scripts/phone11-static-portal-rollout.py"
SPEC = importlib.util.spec_from_file_location("chat_mentions_empty_static_rollout", WRAPPER)
...
SEALED = {
    "RELEASE_SHA": "a" * 40,
    "LIVE_RELEASE_SHA": "60656db3d5d98f59c6f428371061158c8d54deff",
...
class ChatMentionsEmptyStaticRolloutTests(unittest.TestCase):
    def test_actual_release_pins_are_sealed_and_controller_loads(self):
        rollout.validate_pins()
        self.assertEqual(rollout.RELEASE_SHA, "3e8686bdb9a6fe07388eb913c3523140aa425412")
        self.assertEqual(rollout.LIVE_RELEASE_SHA, "60656db3d5d98f59c6f428371061158c8d54deff")
        self.assertEqual(rollout.RELEASE_ARCHIVE_SHA256, "865258470b093f62f8d5c0d411bad81571fdcb9b6af6f920ae4e7610054713cd")
        self.assertEqual(rollout.RELEASE_EXPORT_MANIFEST_SHA256, "e6545cbf2c5171cdd445148eb3cf14e3243677c229c745dd7a66de5f253b3f8a")
        self.assertEqual(rollout.RELEASE_MARKER_SHA256, "a589d4c6d9bd437fe3fd0185fab98b527bc519b83c90da3c072728ba878c2002")
        self.assertEqual(rollout.RELEASE_MAIN_JAVASCRIPT, "_expo/static/js/web/entry-0ca51714385d972ca75b417c992b83d1.js")
        self.assertEqual(rollout.RELEASE_MAIN_JAVASCRIPT_SHA256, "240f85de77ce3be3b9677a1f755a914ea7bd3524d38712b8e33afdf6ca7e555e")
    ...
    def test_exact_controller_bytes_load_with_separate_candidate_pins(self):
        with patch.multiple(rollout, **SEALED):
            ...
            with self.assertRaisesRegex(module.RolloutError, "rollback"):
                module.require_predecessor_pin(module.Release("f" * 40, Path("/tmp/p"), Path("/tmp/m"), "b" * 64, Path("/tmp/current")), "rollback")
