...
const source=():NodeJS.ProcessEnv=>({NODE_ENV:"test",PHONE11_LAB_FCM_SCENARIO_ENABLED:"1",PHONE11_LAB_FCM_ENVIRONMENT:"staging",PHONE11_LAB_FCM_PACKAGE:"ai.phone11.mobile.staging",
 ...
 PHONE11_LAB_SIP_DRIVER_TRANSPORT:"reverse_pull",PHONE11_LAB_SIP_DRIVER_SECRET:"d".repeat(48),PHONE11_WAKE_ENABLED:"1",PHONE11_WAKE_PILOT_SIP_URI:"sip:7101@sip.stage.phone11.test",
...
describe("isolated Phone11 staging FCM routes",()=>{
 ...
 it("fails closed unless the exact staging, execution, Firebase and SIP gates agree",()=>{
  ...
  expect(()=>readLabFcmConfig({...source(),PHONE11_LAB_SIP_DRIVER_TRANSPORT:"direct_https"},now)).toThrowError(LabFcmError);
  expect(()=>readLabFcmConfig({...source(),PHONE11_LAB_SIP_DRIVER_ORIGIN:"https://sip-driver.stage.phone11.ai"},now)).toThrowError(LabFcmError);
 ...
 it("registers the exact HTTP paths with no-store and bounded public errors",async()=>{
  ...
  try{const address=server.address();if(!address||typeof address==="string")throw new Error("missing address");const origin=`http://127.0.0.1:${address.port}`;
   ...
   const privateRunner=await fetch(origin+"/api/phone11/lab/sip-driver/lease",{headers});expect(privateRunner.status).toBe(404);
