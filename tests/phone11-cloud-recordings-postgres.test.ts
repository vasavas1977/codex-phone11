...
describe.skipIf(!socket)('cloud recordings isolated PostgreSQL',()=>{
 ...
 it('retains a safe analysis failure category and clears it on successful retry',async()=>{
  ...
  expect((await pool.query('SELECT failure_code FROM phone11_recording_jobs')).rows[0].failure_code).toBe('provider_rate_limited:generate');
  await pool.query("UPDATE phone11_recording_jobs SET available_at=now()-interval '1 second'");
  const retry=(await repo.claimJob('a'))!;expect(await repo.finishJob(retry,{transcript:'Actual',summary:{summary:'Actual summary',actionItems:[],language:'en'}})).toBe(true);
