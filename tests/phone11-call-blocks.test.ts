...
describe("owner-scoped call blocklist", () => {
  it("normalizes and persists an explicit reason per owner", async () => {
    ...
    expect(JSON.parse(data.get("phone11_call_blocks_v1_user_7")!)).toEqual([
  ...
  it("fails closed on corrupt persisted content", async () => {
    const store = createCallBlocksStore(
      { getItem: async () => '{"not":"a list"}', setItem: async () => undefined },
      () => 7,
    );
    await store.load(7);
    expect(store.getSnapshot()).toMatchObject({
