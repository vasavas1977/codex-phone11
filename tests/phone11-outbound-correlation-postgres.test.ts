...
describe.skipIf(!socket)('observed outbound correlation PostgreSQL',()=>{
 beforeAll(async()=>{
  ...
  await state.pool.query(`CREATE TABLE sip_accounts(id serial PRIMARY KEY,tenant_id integer NOT NULL REFERENCES tenants(id),extension_id integer NOT NULL REFERENCES extensions(id),sip_username text NOT NULL,sip_domain text NOT NULL,status text NOT NULL,deleted_at timestamptz)`);
 ...
 beforeEach(async()=>{
  await state.pool!.query(`TRUNCATE tenants,extensions,sip_accounts CASCADE;
   INSERT INTO tenants(id,name,status) VALUES(10,'A','active'),(20,'B','active');
   INSERT INTO extensions(id,tenant_id,extension_number,status) VALUES(11,10,'3001','active'),(21,20,'4001','active');
   INSERT INTO sip_accounts(tenant_id,extension_id,sip_username,sip_domain,status) VALUES(10,11,'3001','phone11.invalid','active'),(20,21,'4001','other.invalid','active')`);
 });
 ...
 it('creates one exact route and cloud identity under concurrent scanner observations',async()=>{
  ...
  expect((await state.pool!.query('SELECT channel_uuid::text,tenant_id,extension_id,sip_call_id,direction FROM phone11_recording_routes')).rows).toEqual([{channel_uuid:first,tenant_id:10,extension_id:11,sip_call_id:`sip-${first}`,direction:'outbound'}]);
