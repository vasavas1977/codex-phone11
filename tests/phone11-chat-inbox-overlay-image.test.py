...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts/phone11-chat-inbox-overlay-image.py"
spec = importlib.util.spec_from_file_location("chat_inbox_overlay", SCRIPT)
...
class OverlayTests(unittest.TestCase):
    ...
    def test_dockerfile_keeps_one_file_copy_and_disables_inherited_healthcheck(self):
        source = overlay.dockerfile("phone11-chat-inbox-parent:" + "a" * 32)
        self.assertIn("FROM --platform=linux/amd64 phone11-chat-inbox-parent:", source)
