...
describe('authenticated recording ESL',()=>{
 it('allows only bounded prompt filenames in the two durable prompt roots',()=>{
  const make=(announcementPath:string)=>createEslCaptureTransport({host:'127.0.0.1',port:8021,password:'fixture',announcementPath});
  for(const path of ['/opt/phone11ai/prompts/recording.wav','/var/lib/freeswitch/recordings/phone11-prompts/recording-notice-th-en.wav'])expect(()=>make(path)).not.toThrow();
  for(const path of ['/var/lib/freeswitch/recordings/phone11-prompts/../private.wav','/var/lib/freeswitch/recordings/phone11-prompts/sub/notice.wav','/var/lib/freeswitch/recordings/phone11/1/notice.wav','/var/lib/freeswitch/recordings/phone11-prompts/%2e%2e.wav','/var/lib/freeswitch/recordings/phone11-prompts/notice.wav\napi status','/var/lib/freeswitch/recordings/phone11-prompts/notice.mp3','/tmp/notice.wav'])expect(()=>make(path)).toThrow('Invalid capture transport configuration');
 });
