...
const source:NodeJS.ProcessEnv={NODE_ENV:"test",PHONE11_LAB_FCM_SCENARIO_ENABLED:"1",PHONE11_LAB_FCM_ENVIRONMENT:"staging",PHONE11_LAB_FCM_PACKAGE:"ai.phone11.mobile.staging",
 ...
 PHONE11_LAB_FCM_CASES:"PUSH-06",PHONE11_LAB_FCM_TRIGGER_SECRET:"s".repeat(48),PHONE11_LAB_FCM_PUBLIC_ORIGIN:"https://api.stage.phone11.ai",PHONE11_LAB_SIP_DRIVER_ORIGIN:"https://sip-driver.stage.phone11.ai",
...
describe("minimal Phone11 FCM staging Cloud Run service",()=>{
 it("refuses to construct without the existing fail-closed staging gates",()=>{
  ...
  expect(()=>createLabFcmCloudRunApp({source:{...source,PHONE11_LAB_SIP_DRIVER_ORIGIN:"https://sip-driver.stage.phone11.test"},now})).toThrow();
 });
 ...
 it("exposes only health and the two registered lab paths",async()=>{
  const service={attestation:vi.fn(async()=>({environment:"staging"})),trigger:vi.fn(),evidence:vi.fn()} as any;
  ...
  try{const address=server.address();if(!address||typeof address==="string")throw new Error("missing address");const origin=`http://127.0.0.1:${address.port}`;
   ...
   for(const route of ["/api/health","/api/trpc","/api/phone11/wake/offer","/api/recordings"]){const response=await fetch(origin+route);expect(response.status,route).toBe(404);}
