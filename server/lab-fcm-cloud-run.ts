...
import {labFcmService,readLabFcmConfig,registerLabFcmRoutes} from "./push/lab-fcm-routes";
...
export function createLabFcmCloudRunApp(options:Options={}):Express{
 ...
 const config=readLabFcmConfig(source,options.now??Date.now());
 ...
 registerLabFcmRoutes(app,options.service??labFcmService);
