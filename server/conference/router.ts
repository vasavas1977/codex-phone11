...
const action = z.discriminatedUnion("type", [
  z.object({ type: z.literal("mute_participant"), participantId: participant }),
  z.object({
    type: z.literal("unmute_participant"),
    participantId: participant,
  }),
  z.object({ type: z.literal("kick_participant"), participantId: participant }),
  ...(
    [
      "mute_all",
      "unmute_all",
  ...
  z.object({
    type: z.literal("promote_to_moderator"),
    participantId: participant,
  }),
  z.object({
    type: z.literal("demote_to_listener"),
    participantId: participant,
  }),
]);
function service() {
  const db = getPool();
  const provider =
    process.env.PHONE11_CONFERENCE_CONTROLS_ENABLED === "true"
      ? createProvisionedConferenceProvider(
          db,
          createConferenceEsl(getFreeSwitchConfig()),
        )
      : undefined;
...
export const conferenceRouter = router({
  capabilities: protectedProcedure.query(async ({ ctx }) => {
    try {
      return await service().capabilities(ctx.user.id);
    } catch {
      return unavailableCapabilities;
    }
  }),
  list: protectedProcedure.query(({ ctx }) =>
    safe(() => service().list(ctx.user.id)),
  ),
  detail: protectedProcedure
    .input(z.object({ conferenceId: id }))
    .query(({ ctx, input }) =>
      safe(() => service().detail(ctx.user.id, input.conferenceId)),
    ),
  create: protectedProcedure
      ...
      z.object({
        ...
        config: z
          .object({
            ...
            endWhenModeratorLeaves: z.boolean(),
          })
          .partial()
          .optional(),
        idempotencyKey: key,
    ...
    .mutation(({ ctx, input }) =>
      safe(() =>
        service().execute(ctx.user.id, input.idempotencyKey, {
          type: "create",
          name: input.name,
          config: input.config,
        }),
  ...
  addParticipant: protectedProcedure
    .input(
      z.object({
        conferenceId: id,
        extension: z.string().regex(/^[0-9]{2,16}$/),
  ...
  action: protectedProcedure
    .input(z.object({ conferenceId: id, action, idempotencyKey: key }))
    .mutation(({ ctx, input }) =>
      safe(() =>
        service().execute(ctx.user.id, input.idempotencyKey, {
          type: "action",
    ...
    .input(z.object({ conferenceId: id, idempotencyKey: key }))
    .mutation(({ ctx, input }) =>
      safe(() =>
        service().execute(ctx.user.id, input.idempotencyKey, {
          type: "end",
