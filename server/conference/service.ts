...
export interface ConferenceScope {
  userId: number;
  tenantId: number;
}
...
export interface ConferenceRepository {
  ...
  once(
    scope: ConferenceScope,
    key: string,
    operation: ConferenceOperation,
    run: () => Promise<Conference>,
  ): Promise<Conference>;
}
...
export const unavailableCapabilities: ConferenceCapabilities = {
  ...
  videoAvailable: false,
  reason:
    "Conference service is not provisioned for this workspace. Audio room provisioning and verified participant events are required; video media is not configured.",
};
const unavailable = () =>
  new TRPCError({
    code: "PRECONDITION_FAILED",
    message: unavailableCapabilities.reason,
  });
export function createConferenceService(
  repository: ConferenceRepository,
  provider?: ConferenceProvider,
) {
  async function scope(userId: number) {
    return repository.scope(userId);
  }
  return {
    async capabilities(userId: number): Promise<ConferenceCapabilities> {
      ...
      return {
        ...
        joinAvailable: false,
        videoAvailable: false,
        reason: "Video conference media is not configured.",
      };
    },
    ...
    async detail(userId: number, id: string) {
      if (!provider) throw unavailable();
      const actor = await scope(userId);
      await repository.assertRoom(actor, id, false);
    ...
    async execute(userId: number, key: string, operation: ConferenceOperation) {
      ...
      if (operation.type === "create" || operation.type === "addParticipant")
        throw unavailable();
      if ("conferenceId" in operation)
        await repository.assertRoom(actor, operation.conferenceId, true);
      if (
        ...
        [
          ...
          "demote_to_listener",
        ].includes(operation.action.type)
      )
        throw new TRPCError({
          code: "PRECONDITION_FAILED",
          message: "This conference control is not configured",
        });
      return repository.once(actor, key, operation, () =>
        provider.execute(actor, operation),
      );
    },
  };
}
