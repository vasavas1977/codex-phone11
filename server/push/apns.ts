...
export function createApnsSender(deps: Dependencies = {}) {
  ...
  async function request(config: ApnsConfig, token: PushToken, body: string, authorization: string, id: string, timeoutMs: number) {
    return new Promise<void>((resolve, reject) => {
      ...
      try {
        ...
        stream = session.request({ ":method": "POST", ":path": `/3/device/${token.token}`, authorization: `bearer ${authorization}`,
          "apns-topic": `${config.bundleId}.voip`, "apns-push-type": "voip", "apns-priority": "10", "apns-expiration": "0", "apns-id": id, "content-type": "application/json" });
  ...
  return async (token: PushToken, payload: PushPayload, beforeAttempt?: () => Promise<boolean>, deadlineAt?: number): Promise<void> => {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        if (now() >= deadline) throw new PushProviderError("timeout");
        await request(config, token, body, authorization, id, Math.max(1, deadline - now())); return;
      }
      catch (error) {
        ...
        if (!(error instanceof PushProviderError) || error.kind !== "throttled" || error.reason !== "TooManyRequests" || attempt > 0) throw error;
