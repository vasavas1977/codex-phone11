...
export function createWakeRepository(runTransaction: Transaction = withTransaction) {
  ...
  async function authenticate(client: PoolClient, bindingId: string, grant?: string, session?: { id: string; userId: number }) {
    ...
    // binding. Acquire writable authority rows first, then push, then binding
    // explicitly. Canonical identity/assignment rows are read-only to this
    // runtime, so revalidate them without a row lock.
    ...
    async enroll(sessionId: string, userId: number, deviceId: string, pilotUri: string, platform: WakePlatform = "ios") {
      return transaction(async client => {
        ...
        await client.query(`SELECT p.revision FROM phone11_push_devices p ${livePush}
          WHERE p.session_id=$1 AND p.user_id=$2 AND p.device_id=$3 AND p.sip_uri=$4`, [sessionId,userId,deviceId,pilotUri]);
        const result = await client.query(`SELECT p.*,auths."expiresAt" AS session_expiry FROM phone11_push_devices p ${livePush}
          ...
          AND p.sip_uri=$4 FOR SHARE OF p,auths`, [sessionId,userId,deviceId,pilotUri,platform,tokenType]);
