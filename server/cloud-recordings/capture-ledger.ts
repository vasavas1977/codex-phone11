...
export function createCaptureLedger(db= getPool(),repo=createCloudRecordingRepository(db)):CaptureLedger & { pendingUploads():Promise<Array<{channelUuid:string;path:string}>> } {
 ...
 return {
  ...
  async complete(id,file){
   ...
   const result=await db.query(`UPDATE phone11_cloud_recordings r SET capture_stopped_at=COALESCE(capture_stopped_at,clock_timestamp()),capture_upload_lease_token=$4,capture_upload_lease_until=clock_timestamp()+interval '60 seconds'
