...
export async function persistRouteCdr(db:Pool,channelUuid:string,fields:Record<string,string>):Promise<boolean>{
 ...
 try{
  ...
  await c.query(`INSERT INTO call_records(call_uuid,tenant_id,direction,from_number,to_number,started_at,metadata)
   VALUES($1,$2,$3,$4,$5,to_timestamp($6),'{"source":"authenticated_esl_snapshot","completion":"unknown"}') ON CONFLICT(call_uuid) DO NOTHING`,
   [channelUuid,r.tenant_id,r.direction,r.direction==='inbound'?r.number:r.extension_number,r.direction==='inbound'?r.extension_number:r.number,epoch]);
  const parent=await c.query('SELECT id,tenant_id,metadata FROM call_records WHERE call_uuid=$1 FOR UPDATE',[channelUuid]);
  if(parent.rows.length!==1||Number(parent.rows[0].tenant_id)!==Number(r.tenant_id)){await c.query('ROLLBACK');return false;}
  const id=parent.rows[0].id;
  const legs=await c.query('SELECT tenant_id,extension_id,sip_call_id FROM call_legs WHERE call_record_id=$1 AND leg_uuid=$2',[id,channelUuid]);
  if(legs.rows.some(l=>Number(l.tenant_id)!==Number(r.tenant_id)||Number(l.extension_id)!==Number(r.extension_id)||l.sip_call_id!==sip)){await c.query('ROLLBACK');return false;}
  if(!legs.rows.length&&parent.rows[0].metadata?.source!=='authenticated_esl_snapshot'){await c.query('ROLLBACK');return false;}
  if(!legs.rows.length)await c.query(`INSERT INTO call_legs(call_record_id,tenant_id,extension_id,leg_uuid,sip_call_id,started_at,metadata)
   VALUES($1,$2,$3,$4,$5,to_timestamp($6),'{"source":"authenticated_esl_snapshot","completion":"unknown"}')`,[id,r.tenant_id,r.extension_id,channelUuid,sip,epoch]);
