...
async function persistObservedOutbound(observation:OutboundObservation):Promise<boolean>{
 try{return await withTransaction(async c=>{
  const found=await c.query(`SELECT e.id,e.tenant_id FROM sip_accounts s JOIN extensions e ON e.id=s.extension_id AND e.tenant_id=s.tenant_id
   ...
   AND e.status='active' AND e.deleted_at IS NULL AND t.status='active' FOR SHARE OF s,e,t`,[observation.username,observation.realm]);
  ...
  const routes=await c.query(`SELECT tenant_id,extension_id,sip_call_id,direction FROM phone11_recording_routes
  ...
  const exactRoute=(r:any)=>String(r.tenant_id)===String(ext.tenant_id)&&String(r.extension_id)===String(ext.id)&&r.sip_call_id===observation.sipCallId&&r.direction==='outbound';
  ...
  const route=await c.query(`INSERT INTO phone11_recording_routes(channel_uuid,tenant_id,extension_id,sip_call_id,direction,number)
   VALUES($1,$2,$3,$4,'outbound',$5) ON CONFLICT(channel_uuid) DO NOTHING RETURNING channel_uuid`,[observation.channelUuid,ext.tenant_id,ext.id,observation.sipCallId,observation.number]);
