...
export const pbxRouter = router({
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const applyUpdate = async (
          ...
          if (input.voicemailEnabled === true) {
            const oldExtension = oldResult.rows[0] as { user_id: number | null; status: string; type: string };
            const owner = assignedUserId === undefined ? oldExtension.user_id : assignedUserId;
            if (oldExtension.type !== "user" || owner == null ||
  ...
  // ========================================================================
  voicemail: router({
    /** Report schema and media readiness separately; neither proves the relay is commissioned. */
    storageStatus: protectedProcedure
      .input(z.object({ tenantId: z.number().int().positive() }))
      .query(async ({ ctx, input }) => {
        await getTenantAdminCtx(ctx, input.tenantId);
        const liveAdmin = await query(
          `SELECT 1 FROM tenant_memberships tm JOIN tenants t ON t.id = tm.tenant_id
           WHERE tm.user_id = $1 AND tm.tenant_id = $2 AND tm.status = 'active'
             AND tm.role::text IN ('owner', 'admin') AND t.status = 'active'`,
          [ctx.user.id, input.tenantId],
        );
        if (liveAdmin.rows.length !== 1) throw new TRPCError({ code: "FORBIDDEN" });
