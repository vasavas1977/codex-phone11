...
test('all native selectors and delegate methods compile against the exact staged SDK header', {
  skip: process.platform !== 'darwin' ? 'macOS Command Line Tools required' : false,
}, () => {
  assert.ok(existsSync(path.join(framework, 'siprix.framework/Headers/Siprix.h')), 'Stage pinned SDK first');
  run([...base, '-fsyntax-only', path.join(root, 'ios/Phone11Siprix.m')]);
});

test('actual bridge lifecycle, callback, security, and call-control behavior with a mock SDK', {
  skip: process.platform !== 'darwin' ? 'macOS Command Line Tools required' : false,
}, () => {
  ...
  try {
    ...
    run([...base, '-Wno-incomplete-implementation', '-framework', 'Foundation',
      path.join(root, 'tests/native-runtime.m'), '-o', executable]);
    const result = spawnSync(executable, [], { encoding: 'utf8', timeout: 15000 });
    assert.equal(result.status, 0, result.error?.message ?? result.stderr);
    assert.match(result.stdout, /PASS: \d+ native bridge assertions/);
