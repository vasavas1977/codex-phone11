#!/usr/bin/env python3
"""Start an isolated, pinned 3007 API candidate without changing any route.

Run as root on the VoIP host after reviewing the image, source 3006 runtime,
...
SOURCE = "cp11-api-candidate-channel"
TARGET = "cp11-api-candidate-desktop-provisioning"
SOURCE_IMAGE = "sha256:62d8dd798e8bc08b939b75aa57761e1f14341fdbbdcdac7d3a04ae63a6e79dbd"
TARGET_IMAGE = "sha256:b714cec08a15f6465baba58204a3473494e8c043bd3cef5323b5a35d676f6baa"
SOURCE_BUNDLE = "a846c1c73ae3eeb55f4f4a60b3266981fad980ded33dc331c5ccd2dbb79f56c4"
TARGET_BUNDLE = "8dda0429b527238d96e742f03e8c2f8b523247c705b5d3cf9350c0a57e699897"
BUILD = "desktop-provisioning-20260924"
NETWORK = "cloudphone11-prod_cp11-net"
STAGING = Path("/opt/phone11ai/desktop-provisioning-20260924")
EXPECTED_MOUNTS = {
...
def main() -> None:
    ...
    if source["Image"] != SOURCE_IMAGE or source["State"]["Status"] != "running":
        raise RuntimeError("source 3006 runtime changed")
    if image["Id"] != TARGET_IMAGE or image["Config"]["Labels"].get("com.phone11.bundle-sha256") != TARGET_BUNDLE:
    ...
    if source_bundle != SOURCE_BUNDLE:
        raise RuntimeError("live 3006 bundle changed")
    ...
    with socket.socket() as sock:
        sock.settimeout(0.4)
        if sock.connect_ex(("127.0.0.1", 3007)) == 0:
            raise RuntimeError("port 3007 is occupied")

    config = source["Config"]
    host = source["HostConfig"]
    for field in ("User", "Entrypoint", "Cmd", "WorkingDir"):
        if image["Config"].get(field) != config.get(field):
            raise RuntimeError(f"candidate image runtime {field} differs from 3006")
    ...
    if host["PortBindings"] != {"3006/tcp": [{"HostIp": "127.0.0.1", "HostPort": "3006"}]}:
    ...
    if variables.get("PHONE11_RUNTIME_ROLE") != "api-candidate" or variables.get("PORT") != "3006":
    ...
    args = ["docker", "create", "--name", TARGET, "--restart", "no",
            "--network", NETWORK, "--network-alias", "candidate_desktop_provisioning",
            "--memory", "2147483648", "-p", "127.0.0.1:3007:3007"]
    for item in mounts:
    ...
    health = ("fetch('http://127.0.0.1:3007/api/health')"
    ...
    try:
        ...
        for _ in range(30):
            try:
                with urlopen("http://127.0.0.1:3007/api/health", timeout=2) as response:
                ...
                if data.get("ok") is True and data.get("runtimeRole") == "api-candidate" and data.get("build") == BUILD:
                    ...
                    print(f"candidate_ready image={TARGET_IMAGE} bundle={TARGET_BUNDLE} port=3007")
                    return
            except Exception:
                pass
            time.sleep(1)
        raise RuntimeError("candidate health did not become ready; route remains on 3006")
    except Exception:
