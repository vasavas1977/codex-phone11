#!/usr/bin/env python3
"""Build and audit a one-file chat-inbox image from the pinned live image.
...
PARENT_IMAGE = "sha256:c66d4e95e4177a75bb4a3c0c6a900de80c57f071b0b3d8d7daa45332404b2398"
PARENT_BUNDLE = "870fdad722679a67575a57c27fee5131c2128aa75d97d201064588c76cb0292e"
SOURCE_SHA = "2d125819a7f1713f05b3eaa3bcbf5e5672a84e0c"
BUNDLE_SHA = "75381c01555e1d924eddc2da2c97a1f1e44224dec5c4f03947518e24f6148b5b"
BUILD = "chat-inbox-20260925"
SOURCE_CONTAINER = "cp11-api-candidate-meeting-title"
...
def dockerfile(parent_tag: str) -> str:
    if not re.fullmatch(r"phone11-chat-inbox-parent:[0-9a-f]{32}", parent_tag):
    ...
    return (
        ...
        f"LABEL {labels}\n"
        # The 3010 start operator supplies its own health check; do not retain
        # the parent's 3000/3009 health probe in the candidate image.
        "HEALTHCHECK NONE\n"
    )
...
def build(bundle_path: Path) -> str:
    ...
    temporary_tag = f"phone11-chat-inbox-parent:{uuid.uuid4().hex}"
    context_dir = Path(tempfile.mkdtemp(prefix="phone11-chat-inbox-build-"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bundle-path", type=Path, required=True,
                        help="path to the reviewed chat-inbox backend bundle")
