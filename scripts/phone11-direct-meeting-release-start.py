#!/usr/bin/env python3
"""Start an isolated, pinned 3011 direct-meeting API candidate without changing any route.

Run as root on the VoIP host after reviewing the image, source 3010 runtime,
...
SOURCE = "cp11-api-candidate-chat-inbox"
TARGET = "cp11-api-candidate-direct-meeting"
SOURCE_IMAGE = "sha256:55b593f0c392c67bc74589dcae4cb0e36b2f2e6dcd8a3552be781429ed0e2d77"
SOURCE_BUNDLE = "75381c01555e1d924eddc2da2c97a1f1e44224dec5c4f03947518e24f6148b5b"
TARGET_SOURCE_SHA = "2bbb1ed2e49fb6c1ede1e1aa443eba4fc98238f1"
TARGET_BUNDLE = "b42e1b4a88cf063b69aa2faa66fcd944224f5f2f66f4302fdee36513afe3aa74"
BUILD = "direct-meeting-20260926"
NETWORK = "cloudphone11-prod_cp11-net"
STAGING = Path("/opt/phone11ai/direct-meeting-20260926")
EXPECTED_MOUNTS = {
...
def create_with_memory_environment(args: list[str], variables: dict[str, str], image: str) -> str:
    ...
    fd = os.memfd_create("phone11-direct-meeting-candidate-env", os.MFD_CLOEXEC)
    try:
...
def main(target_image: str) -> None:
    ...
    if source["Image"] != SOURCE_IMAGE or source["State"]["Status"] != "running":
        raise RuntimeError("source 3010 runtime changed")
    ...
    if source_bundle != SOURCE_BUNDLE:
        raise RuntimeError("live 3010 bundle changed")
    with urlopen("http://127.0.0.1:3010/api/health", timeout=5) as response:
        raw = response.read(16_385)
        if response.status != 200 or len(raw) > 16_384:
            raise RuntimeError("source 3010 health changed")
        health = json.loads(raw)
        if health.get("ok") is not True or health.get("service") != "phone11-backend" or health.get("build") != "chat-inbox-20260925" or health.get("runtimeRole") != "api-candidate":
            raise RuntimeError("source 3010 health changed")
    ...
    with socket.socket() as sock:
        sock.settimeout(0.4)
        if sock.connect_ex(("127.0.0.1", 3011)) == 0:
            raise RuntimeError("port 3011 is occupied")

    config = source["Config"]
    host = source["HostConfig"]
    for field in ("User", "Entrypoint", "Cmd", "WorkingDir"):
        if image["Config"].get(field) != config.get(field):
            raise RuntimeError(f"candidate image runtime {field} differs from 3010")
    ...
    if host["PortBindings"] != {"3010/tcp": [{"HostIp": "127.0.0.1", "HostPort": "3010"}]}:
    ...
    if variables.get("PHONE11_RUNTIME_ROLE") != "api-candidate" or variables.get("PORT") != "3010":
    ...
    args = ["docker", "create", "--name", TARGET, "--restart", "no",
            "--network", NETWORK, "--network-alias", "candidate_direct_meeting",
            "--memory", "2147483648", "-p", "127.0.0.1:3011:3011"]
    for item in mounts:
    ...
    health = ("fetch('http://127.0.0.1:3011/api/health')"
    ...
    try:
        ...
        for _ in range(30):
            try:
                with urlopen("http://127.0.0.1:3011/api/health", timeout=2) as response:
                ...
                if data.get("ok") is True and data.get("runtimeRole") == "api-candidate" and data.get("build") == BUILD:
                    ...
                    print(f"candidate_ready image={target_image} bundle={TARGET_BUNDLE} port=3011")
                    return
            except Exception:
                pass
            time.sleep(1)
        raise RuntimeError("candidate health did not become ready; route remains on 3010")
    except Exception:
