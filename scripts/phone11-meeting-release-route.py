#!/usr/bin/env python3
"""Guarded route switch for the Phone11 meeting API candidate.

Run as root on the VoIP host after a separately reviewed candidate is already
healthy on loopback port 3008. Only the two tRPC proxy_pass directives move.
...
SCHEMA = "phone11-meeting-release-route/v1"
SITE = Path("/etc/nginx/sites-enabled/phone11ai")
STATE_ROOT = Path("/var/lib/phone11-meeting-release-route")
# Keep the same lock used by the previous Phone11 route operator.
LOCK = Path("/run/phone11-desktop-provisioning-route.lock")
ORIGINAL_SHA256 = "66e18ffe1a93f643c541c501620b184fd31c2ce8958f1920947156ebe7e4b81f"
CURRENT_CONTAINER = "cp11-api-candidate-desktop-provisioning"
CURRENT_IMAGE = "sha256:b714cec08a15f6465baba58204a3473494e8c043bd3cef5323b5a35d676f6baa"
CURRENT_BUNDLE = "8dda0429b527238d96e742f03e8c2f8b523247c705b5d3cf9350c0a57e699897"
TARGET_CONTAINER = "cp11-api-candidate-meetings"
TARGET_BUNDLE = "97770ec21c24ec80f235545ffd66371eb9dd8fd756444578a6af09a5640c2d3b"
TARGET_BUILD = "meeting-admin-20260925"
CURRENT_PORT = 3007
TARGET_PORT = 3008
...
def activate(image: str, bundle: str, build: str) -> None:
    ...
    require(bundle == TARGET_BUNDLE and build == TARGET_BUILD, "candidate_pin")
    with lock():
        site, info = read_regular(SITE)
        require(info.st_uid == 0 and info.st_gid == 0, "site_owner")
