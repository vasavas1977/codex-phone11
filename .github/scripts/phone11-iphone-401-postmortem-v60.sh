...
KEY_PATH="${HOME}/.ssh/phone11_v60_key"
...
USER_NAME="1001"
START="2026-05-24T15:56:45Z"
END="2026-05-24T15:58:15Z"

echo "===== V60 exact iPhone 401 postmortem ====="
echo "app_build=986d99f5-8162-4ba8-9e2c-2670c1bc19f0"
echo "app_commit=0f76af528fc04fa539959e17b7abc2f8d6bb58d9"
echo "app_401_time_utc=2026-05-24T15:57:33.580Z"
echo "server_window_start=$START"
echo "server_window_end=$END"
echo "target_user=$USER_NAME"
echo "target_domain=$DOMAIN"
...
echo "===== Current location row for 1001 ====="
"${PSQL_BASE[@]}" -c "SELECT username, domain, contact, received, user_agent, expires FROM location WHERE username='1001' OR contact ILIKE '%1001%' ORDER BY expires DESC NULLS LAST LIMIT 10;" || true

echo "===== Kamailio logs around exact app 401 window ====="
if docker ps --format '{{.Names}}' | grep -qx p11-kamailio; then
  docker logs --since "$START" --until "$END" --timestamps p11-kamailio 2>&1 \
    | grep -Eia 'REGISTER|1001|401|407|200 OK|Unauthorized|auth|nonce|digest|error|warning|failed|received|reply|sip.phone11.ai' \
    | sed -E 's/(nonce=")[^"]+(")/\1<redacted>\2/g; s/(response=")[^"]+(")/\1<redacted>\2/g; s/(cnonce=")[^"]+(")/\1<redacted>\2/g; s/(password=)[^ ,;]+/\1<redacted>/g' \
    | tail -n 500 || true
else
  echo "p11-kamailio container missing"
fi

echo "===== Host auth/syslog around exact app 401 window ====="
sudo journalctl --since "$START" --until "$END" --no-pager 2>/dev/null \
  | grep -Eia 'kamailio|REGISTER|1001|401|407|Unauthorized|auth|digest|sip.phone11.ai' \
  | sed -E 's/(nonce=")[^"]+(")/\1<redacted>\2/g; s/(response=")[^"]+(")/\1<redacted>\2/g; s/(cnonce=")[^"]+(")/\1<redacted>\2/g; s/(password=)[^ ,;]+/\1<redacted>/g' \
  | tail -n 500 || true
...
echo "===== Synthetic REGISTER using current live password ====="
set +e
PHONE11_TEST_PASS="$PASS" python3 - <<'PY'
...
def digest(ch, method, uri):
    ...
    print(f'V60_CHALLENGE_REALM={realm}')
    print(f'V60_CHALLENGE_QOP={qop or "none"}')
...
sock.bind((IP, random.randint(29000,29999)))
sock.setblocking(False)
sport=sock.getsockname()[1]
cid=f'phone11-v60-{secrets.token_hex(8)}@ec2'
uri=f'sip:{DOMAIN}'
tag=secrets.token_hex(6)
def send(cseq, auth=''):
    lines=[
        f'REGISTER {uri} SIP/2.0',
        f'Via: SIP/2.0/UDP {IP}:{sport};branch=z9hG4bKv60{secrets.token_hex(6)};rport',
        'Max-Forwards: 70',
        ...
        'Expires: 300',
        'User-Agent: Phone11V60AuthEvidence',
    ]
    if auth:
        lines.append(auth)
...
print(f'V60_SYNTH_CALL_ID={cid}')
send(1)
first=recv()
c,r=code(first)
print(f'V60_FIRST_STATUS={c}')
print(f'V60_FIRST_REASON={r}')
ch=header(first,'WWW-Authenticate') or header(first,'Proxy-Authenticate')
print(f'V60_HAS_CHALLENGE={bool(ch)}')
...
second=recv()
c,r=code(second)
print(f'V60_FINAL_STATUS={c}')
print(f'V60_FINAL_REASON={r}')
print('V60_SYNTH_REGISTER_RESULT=' + ('pass' if c==200 else 'fail'))
sys.exit(0 if c==200 else 42)
PY
PY_EXIT=$?
set -e
echo "synthetic_exit=$PY_EXIT"
echo "V60_POSTMORTEM_COMPLETE=yes"
exit "$PY_EXIT"
REMOTE
