...
KEY_PATH="${HOME}/.ssh/phone11_v50_key"
...
OUT=/tmp/phone11-v50-register.out
...
echo "===== Subscriber auth row, redacted ====="
docker exec -i -e PGPASSWORD="$PGPASSWORD_VALUE" cp11-postgres psql -v ON_ERROR_STOP=1 -U "$PGUSER" -d "$PGDATABASE" <<SQL
SELECT username, domain, length(coalesce(password,'')) AS password_len, length(coalesce(ha1,'')) AS ha1_len, length(coalesce(ha1b,'')) AS ha1b_len,
...
echo "===== sip_accounts columns only, no secrets ====="
docker exec -i -e PGPASSWORD="$PGPASSWORD_VALUE" cp11-postgres psql -v ON_ERROR_STOP=1 -U "$PGUSER" -d "$PGDATABASE" <<SQL
SELECT column_name FROM information_schema.columns WHERE table_name='sip_accounts' ORDER BY ordinal_position;
SQL
...
if [ -z "$PASS" ]; then
  echo "V50_BLOCKER=no_subscriber_plaintext_password"
  exit 31
fi
...
def digest(ch, method, uri):
    ...
    print(f'V50_CHALLENGE_REALM={realm}')
    print(f'V50_CHALLENGE_QOP={qop or "none"}')
...
cid=f'phone11-v50-{secrets.token_hex(8)}@ec2'
uri=f'sip:{DOMAIN}'
tag=secrets.token_hex(6)
def send(cseq, auth=''):
    lines=[
        f'REGISTER {uri} SIP/2.0',
        f'Via: SIP/2.0/UDP {IP}:{sport};branch=z9hG4bKv50{secrets.token_hex(6)};rport',
        'Max-Forwards: 70',
        ...
        'Expires: 300',
        'User-Agent: Phone11V50AuthEvidence',
    ]
    if auth:
        lines.append(auth)
...
print(f'V50_SYNTH_CALL_ID={cid}')
send(1)
first=recv()
c,r=code(first)
print(f'V50_FIRST_STATUS={c}')
print(f'V50_FIRST_REASON={r}')
ch=header(first,'WWW-Authenticate') or header(first,'Proxy-Authenticate')
print(f'V50_HAS_CHALLENGE={bool(ch)}')
...
second=recv()
c,r=code(second)
print(f'V50_FINAL_STATUS={c}')
print(f'V50_FINAL_REASON={r}')
print('V50_SYNTH_REGISTER_RESULT=' + ('pass' if c==200 else 'fail'))
...
cat "$OUT" || true
echo "V50_POSTMORTEM_COMPLETE=yes"
exit "$PY_EXIT"
REMOTE
