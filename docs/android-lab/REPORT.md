...
- L3 delivery remains blocked after client commissioning: authenticated backend token enrollment and wake binding have not been completed, the isolated scenario/evidence backend is not deployed, an FCM HTTP v1 service-account sender is not available, and no real provider send or delivery case has run. No fake notification or local broadcast was counted as Firebase evidence.
