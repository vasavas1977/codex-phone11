...
- Commissioned Firebase staging candidate: source commit `d908e9c1a9ed1d41c87b67bdcae386e515953842`; APK `.lab/Phone11-Android-Staging-1.0.0-d908e9c.apk`; SHA-256 `2824e7daf148ba822a677b8b899a8ec354f9fc056ceac62c5d462a14b0a275e3`; application ID `ai.phone11.mobile.staging`.
...
The exact final APK now contains the default-off authenticated Android enrollment chain. The shared client obtains an FCM token only after the native staging/Firebase capability passes, registers `platform=android` and `tokenType=fcm` under the captured authenticated session, enrolls and resolves the exact wake binding, saves the public binding in the native bridge, rotates register-first, and clears native/token state on logout. Raw provider tokens remain transient; the server-issued wake grant is encrypted with Android Keystore AES-GCM and never returned by the public binding query. The server wake path selects only the matching Android-FCM or iOS-VoIP pair and sends Android a data-only envelope containing exactly version, call UUID, binding UUID and expiry. The existing iOS APNs branch is unchanged. Without Firebase configuration, all Android provider capabilities and Phone11 services remain disabled.

An isolated Firebase project and Android app now exist for `phone11-stage-20260914` / `ai.phone11.mobile.staging`. The exact commissioned APK was built, independently verified and installed on `emulator-5580`. Its on-device diagnostic initially reported the provider token unavailable while the emulator had no usable DNS; after restoring emulator DNS, Firebase registration succeeded. Evidence records only `tokenPresent=true` and the non-reversible short SHA-256 fingerprint `50a22f87d237359a`. No raw registration token was placed in UI, logs or evidence.
...
The commissioned staging candidate passed the exact-APK verifier: package `ai.phone11.mobile.staging`, source commit `d908e9c1a9ed1d41c87b67bdcae386e515953842`, APK SHA-256 `2824e7daf148ba822a677b8b899a8ec354f9fc056ceac62c5d462a14b0a275e3`, Firebase project `phone11-stage-20260914`, sender `413228367517` and Android app ID `1:413228367517:android:f41353883923fc15911e74`. Both `Phone11IncomingCallService` and `Phone11FirebaseMessagingService` are enabled and non-exported. Installation and cold launch on `emulator-5580` produced the safe Firebase fingerprint above. This proves client commissioning and registration-token availability; it is not provider-send or delivery evidence.

## Open gates

- L3 delivery remains blocked after client commissioning: the isolated scenario/evidence routes and strict real-SIP driver contract are implemented at `97c8406` but not deployed, authenticated backend token enrollment and wake binding have not been completed, an FCM HTTP v1 service-account sender is not available, and no real provider send or delivery case has run. No fake notification or local broadcast was counted as Firebase evidence.
...
- Commissioned staging APK identity: `.lab/staging-apk.json`, `.lab/staging-apk-verification.json`; installed diagnostic: `.lab/staging-firebase-diagnostic-d908e9c.xml`, `.lab/staging-firebase-diagnostic-d908e9c.png`
...
The commissioned client-side Firebase prerequisites are now proven through real on-device registration, and the isolated backend route contract is implemented locally. The next Android acceptance gate is deployment plus authenticated token enrollment and wake binding with an authorized staging service-account sender. After those pieces exist, the lab can perform the first real provider send and preserve separate evidence for provider acceptance, foreground/background receipt, lock, Doze, notification ownership, SIP adoption and true process death. Full-screen use additionally requires a deliberate permission and policy decision. The next iOS gate is the signed daily-pilot candidate and physical playback/seek/speaker check; recovering the12:40 summary additionally requires approval for one production Gemini retry after the diagnostic backend change is deployed.
