...
| `docs/manus-diagnostics/DEPLOYMENT_REPORT.md` | Deployment report with environment-specific connection material | Recreate from live infrastructure without secrets |
...
| `server/pbx/db.ts` | module initialization | PBX database credentials | Runtime env only |
| `server/phone-provisioning.ts` | module initialization | PBX database credentials | Shared runtime env only |
| `infra/configs/kamailio/kamailio.cfg` | `DB_URL` substdef | Kamailio database URL | Placeholder URL |

## Files SAFE (already use env vars or template placeholders)

| File Path | Notes |
|-----------|-------|
| `infra/configs/kamailio/kamailio.cfg` | Uses placeholder `DB_URL` and `KAM_SECRET` substdefs — values must be injected at deploy time |
...
| `*.bak*` | Backup config files that can preserve old credentials |
| Environment-specific deployment reports | Can preserve credentials, public IPs, and private service topology |
