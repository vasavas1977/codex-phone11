...
site and `nginx -T`: `localhost`, IPv6 loopback, upstream aliases, and dynamic
destinations that could reach an inactive candidate are refused. It seals the
source Compose bytes and render, protected probe hash, Nginx site and complete
dump hashes, Kamailio hash, image source/bundle/lock labels, and all five
...
For v3, inventory accepts a root-only applied receipt only at
`/var/lib/phone11-channel-meetings/receipt.json`. Its exact SHA-256, schema,
catalog proof, and verification hash are sealed into the manifest. `--prepare`
rechecks it before any target or image action. The manifest has one explicit
tenant ID, which must equal the existing protected probe tenant ID. The target
environment receives exactly two additional values:
...
conflict refusal including `localhost` and effective-upstream target aliases,
v3 settings capability probe, applied migration-receipt refusal, exact target
