...
The integrated backend candidate preserves the deployed desktop provisioning overlay and includes the tenant-first admission lock change. Its 90 local meeting tests, TypeScript and backend bundle pass. Independent review also found an extension-revocation race; the separate concurrency workstream is fixing and proving that path against disposable local PostgreSQL before any release.
