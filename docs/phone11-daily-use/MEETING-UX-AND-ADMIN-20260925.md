...
Desktop shared-screen video is now mounted separately from participant cameras, with `object-fit: contain` and SDK attachment cleanup on replacement, unpublish and leave. Camera-off participants can keep sharing without their screen being removed by roster updates. Missing display names use “Participant”, never opaque provider identity; completing an authenticated name mapping remains a service contract gap. This update passed 19 desktop tests, TypeScript and the Electron build locally. It is not yet the staged or installed desktop artifact.
