...
**Installation is pending.** The paired iPhone was no longer available to the device service when installation was attempted. Although the installer returned exit code zero, its log reported Device not found and the independent inventory could not locate the phone; this is not installation success. The last verified installed version remains build 15. Reconnect and unlock the physical phone, install build 17, verify its inventory, then run the bounded incoming echo test with Phone11 open. Build 16 is superseded and should not be installed.
