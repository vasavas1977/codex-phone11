...
The photo capability and public upload/read/delete routes now require `PHONE11_PROFILE_PHOTO_COMMISSIONED=1` on the API server. It defaults off; any other value remains off. Set it only after the public `/api/profile/photo` routes, both photo tables, writable private media storage, and the photo retention/cleanup owner are deployed and verified. This setting is an operator assertion, not a runtime check of the public route or worker. Leave it unset until those checks pass; after enabling, verify the authenticated capability and upload/read/remove path for an active member and denied access from another workspace. Private cleanup work remains independent of this public gate.
