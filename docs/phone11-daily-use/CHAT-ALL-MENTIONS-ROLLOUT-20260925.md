...
In the authenticated web Test channel, typing `@` displayed the live `Mention everyone` / `@all` option and the channel-member picker for the signed-in account. The draft was cleared without sending a message. This verifies the allowed-account picker path only. Ordinary-member rejection, push delivery, and two-handset behavior have not been exercised on the live tenant in this operation; those checks remain distinct from the catalog and picker proof.
