...
The opening implementation, verification and deployment sections record the first release on 10 September 2026. The latest independently verified installation is [build 23](#signed-and-installed-candidate-build-23), source `f21e763`. Build 23 now has a bounded foreground public-number two-way audio acceptance: after the matched PCMA-only test, the user confirmed “ok working now.” The tested audio setting is retained only for the dedicated Phone11 number. The first two inconclusive attempts were rolled back; the third was matched and accepted. See [the public audio fix](PUBLIC-AUDIO-FIX-20260911.md). Build 19 direct echo remains separate historical evidence.

The latest signed artifact is **build 25**, the separate daily-use pilot, application source `1b004ba`. Its signed production APNs entitlement, signing identity and call/chat pilot configuration are verified, but it is **not installed** and does not establish background-call or recipient-notification acceptance. Build 23 remains installed. See [the current readiness checkpoint](DAILY-READINESS-20260912.md). Build 24 remains the preceding verified call-wake-only artifact.
...
Earlier calls to 02-030-3001 were silent or one-way despite packet forwarding, signal energy and carrier reception reports. A third, matched PCMA-only test on build 23 subsequently passed the bounded foreground public-number audio gate: the user confirmed “ok working now,” and the call ended with BYE. The tested setting remains active only for that dedicated number. See [the accepted fix and its limits](PUBLIC-AUDIO-FIX-20260911.md); this does not establish other callers, networks, call durations or background states.
...
Additional public-number calling scenarios, native PushKit/APNs delivery while locked/backgrounded, production Siprix licensing and long calls, two authorized chat clients, and remaining physical audio-route/control tests are separate acceptance gates. Background calling has a disabled integrated source candidate included in installed build 23; approved deployment/commissioning and physical acceptance remain required. Production credential revocation may require additional engineering. These are not credentials-only blockers.
...
The native commissioning flag remains zero and server wake enablement defaults off. Build 21's public-number tests failed two-way hearing; installed build 23 subsequently passed one matched foreground PCMA-only test with user confirmation. The first two inconclusive canaries were rolled back, while the third tested setting is retained for the dedicated number. Apple delivery, locked-device calling, long licensed calls and two-person chat remain unaccepted.
...
### PCMA interoperability tests — third attempt accepted and retained
...
The original configuration was restored after fresh zero-active-call checks, and health was verified. Private rollback evidence confirms both host and container SHA256 `bb9168b3a9c312ea05267af290c3d9e581a5b31c199f905bfb410c8e986ae378`. That first attempt was no longer active after rollback. The private packet/level/rollback artifacts retain the detailed evidence; no caller identity or device identifier is published here.

A second PCMA canary attempt after build 23 installation did not monitor a call: after the restart, the monitor's guard requiring exactly one fresh pilot registration contact failed, so the capture did not start. This is another **inconclusive** attempt, not a codec or audio result. The second rollback was independently verified from private evidence as `rolled-back`, with both host and container restored to the same baseline SHA256 above. A subsequent fresh check confirmed the baseline and one pilot contact with 238 seconds remaining at that observation. This is a point-in-time readiness check, not a permanent registration guarantee.

A subsequent bounded monitor captured an exact build 23 call through BYE with zero capture drops, without changing the baseline configuration. It observed **1,039 outgoing PCMA packets**, continuous sequence numbers and expected adjacent timestamp progression, with nonzero signal energy. Four carrier RTCP reception reports matched that outgoing stream's SSRC and observed sequence range; three advanced beyond the preceding report, and all reported zero loss. This is evidence of carrier-side packet receipt, not intelligible speech or handset playout. The user still reported one-way hearing. This unchanged-baseline call remained one-way and preceded the third PCMA-only test below.


On the third attempt, the PCMA-only configuration was reapplied with deliberate re-registration and a matching call was captured. Both directions negotiated PCMA as the only voice codec, preserving telephone-event. The trace observed 1,245 outgoing and 1,260 incoming PCMA packets, seven carrier reception reports (five advancing) with zero reported loss, BYE and zero capture drops. There was one transient outgoing sequence-gap/reordering observation; the trace does not support an absolute zero-loss or zero-reordering claim. Immediately after this test, the user confirmed **“ok working now.”** The tested setting is **active and retained** for the dedicated Phone11 number; original configuration backups remain private. See [PUBLIC-AUDIO-FIX-20260911.md](PUBLIC-AUDIO-FIX-20260911.md) for exact hashes, validation and guarded recovery.
...
After the iPhone reconnected over USB, build 23 installed successfully. The installer reported completion, and independent device inventory confirms the expected application identity, version **1.0.0** and build **23**. Automated launch was rejected by the iOS process-control service, but a subsequent physical call connected and the End action reached the terminal event in **16 milliseconds** in the captured handset diagnostics. These observations establish control behavior for that call; the user still reported that Phone11 heard the caller while the caller heard nothing. That earlier call did not pass audio acceptance. The later third PCMA-only test passed bounded foreground two-way audio with the user's confirmation, and the dedicated-number setting is retained. The live backend remains `75fa3c9`; native/server background calling remains disabled.


### Signed pilot: build 24 — not installed

Source `da4187df3435a390245036b115c83feb97495eb0` passed [workflow 34624514614](https://github.com/vasavas1977/codex-phone11/actions/runs/34624514614). EAS build `73bdf19f-a244-48c0-82e8-58e4a478a8dc` finished for `preview-ios-siprix-wake-pilot`. Version **1.0.0/build 24** is **16,607,040 bytes**, SHA256 `a540c6cf6a3d21b4742a300632cebfa69cadb3734aba2547009f5815fe2dc2a6`. Content length and ZIP integrity passed. The main executable's actual signed APNs entitlement and the provisioning profile both specify production; application identity, team and signing certificate match build 23.

The artifact contains wake gate 1 in its native plist and the isolated runtime `1.0.0-siprix-wake-pilot-1`. Updates are disabled, with no embedded channel header; the selected source profile specifies the separate pilot channel. The ordinary build remains disabled. No Siprix license is configured in the pilot artifact. Build 24 has not been installed or used for calls; **build 23 remains the latest verified installation**. These checks do not prove native wake delivery, background/locked Answer or audio, or licensed long calls. See [the pilot commissioning requirements](BACKGROUND-PILOT-BUILD-20260911.md).


### Later daily-use source — 12 September

Global foreground chat refresh now keeps the Team badge current on other tabs while preserving pending sends and drafts across background transitions. Legacy notification-center, billing and conference routes no longer display fabricated data. Gated message-alert settings request actual iOS permission and report the result of real device enrollment.

The ordinary-message notification implementation has its own session/workspace-scoped registry and transactional outbox, separate from VoIP push. Generic lock-screen alerts contain no message text, sender or conversation details. Fresh authorization resolves taps; duplicate and superseded tap callbacks cannot open the wrong conversation. Provider errors cannot alter an already committed message. A distinct daily pilot build profile keeps this source isolated from the default builds.

This source is not installed or deployed. The separate ordinary-alert migration remains unapplied. The earlier call-wake migration completed with a full restore rehearsal and exact protected-data checks, and a private frozen-backend candidate passed health and unauthenticated/CORS checks before being stopped pending real owner sign-in. Live backend promotion, APNs configuration, locked-phone calls, ordinary recipient notifications, a production SDK license and long-call acceptance remain open. Build 23 is still the installed app; build 24 remains the verified signed call-wake artifact. See [the current preparation and pilot evidence](BACKGROUND-PILOT-BUILD-20260911.md).


### Signed daily pilot: build 25 — not installed

Exact application source `1b004bac738111d1ba7849e627341f641cea3af1` passed all required native, database, calling, chat and actual prebuild checks in [workflow 34633414125](https://github.com/vasavas1977/codex-phone11/actions/runs/34633414125). EAS `207befc2-ca78-44b9-a252-0fbdb962e7cd` finished; build 25 is 16,572,310 bytes, SHA256 `d29afc67a2a88cec6b040dbc613df4a57b15fa5581c55f2e4497c7138bb0ce2c`. Signed production APNs and the provisioning identity/certificate match installed build 23. The daily pilot contains wake/chat markers 1/1 and isolated runtime `1.0.0-siprix-daily-pilot-1`, with updates disabled. It has no configured Siprix license.

The final enrollment repair adds active-account/workspace-scoped backoff for temporary setup failures and consumes native token events without recursively requesting tokens. Sixty-four focused tests passed; 49 overlapping checks passed independent review. The final iPhone export also passed. Build 25 is not installed; no new provider delivery or physical call/message acceptance was performed. See [the concise owner inputs and remaining gates](DAILY-READINESS-20260912.md).
