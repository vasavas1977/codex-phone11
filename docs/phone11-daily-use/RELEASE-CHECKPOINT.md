...
The opening implementation, verification and deployment sections record the first release on 10 September 2026. The latest signed and artifact-verified candidate is [build 23](#signed-candidate-build-23--installation-pending), source `f21e763`; installation is pending. Build 21 remains the latest verified installation, while the accepted direct echo remains build 19 evidence. Public-number one-way audio remains unresolved; the temporary PCMA canary was inconclusive and has been rolled back.
...
The integrated daily-use candidate was signed and installed as build 21 from `54923dc9583987fa5e1ee0bd5cb0407a8ff9c6c4`. Its public-number tests below have not achieved two-way audio acceptance. Later registration and incoming-screen fixes are signed and artifact-verified as build 23 from `f21e763`; build 23 is not installed. See [build 21 installation evidence](#signed-and-installed-candidate-build-21) and [the latest signed candidate](#signed-candidate-build-23--installation-pending).
...
Build 23 is **not installed**: the device was absent at the latest installation-readiness check. Signing/artifact verification is complete, while installation and physical acceptance remain open. The latest verified handset build is still **21**. The live backend remains `75fa3c9`, the PCMA canary is rolled back, and native/server background calling remains disabled.
