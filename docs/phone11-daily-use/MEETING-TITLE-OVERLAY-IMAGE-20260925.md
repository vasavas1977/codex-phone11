...
The candidate source is `d67be349fbedbda0d4d441c3e0d958ae1481611a`; its bundled server file is SHA-256 `870fdad722679a67575a57c27fee5131c2128aa75d97d201064588c76cb0292e`. The active parent image is `sha256:d23a97bd859bb2788802fe50debc0d5551a1125d3b879d62277016f41fa0bd22`. The first host attempt failed because Docker 29.1.3 has no buildx plugin. A second attempt with the classic builder created a dangling image, but the audit rejected it before acceptance: Docker saved its last layer as a gzip-compressed OCI blob, while the old check compared the compressed blob hash to the uncompressed RootFS diffID. The rejected image `sha256:e45cdbffb1a89b6aa3b8919dce058447b229195c6145dde5b1ec817d65fcabc2` must not be used for the 3009 start operator. This revision has not been rerun on the host.
...
Passing those checks is source evidence only. A newly accepted image ID and live Docker verification remain pending until this corrected builder runs on the VoIP host. The 3009 candidate, route change, authenticated title behavior, and two-device meeting audio each need their own later evidence.
