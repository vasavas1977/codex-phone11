...
- My profile now treats the service's `PRECONDITION_FAILED` status as an uncommissioned workspace capability instead of suggesting a network retry. Build 104 displayed “Workspace status is not enabled for this workspace yet” with no misleading Retry action; the profile photo remained visible.
