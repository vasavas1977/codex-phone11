...
Build 18 includes the navigation and diagnostic changes described above. No production SDK license is embedded, background calling remains uncommissioned, and the live backend remains on `75fa3c940aa983cff30ed967d444c73b43cc9a53`. After renewed readiness, the bounded test was placed with a 35-second ringing window, an independently accepted 55-second safety guard and device-screen capture while ringing. The call connected through the in-app Answer action and the handset ended it after approximately 17 seconds, before cleanup. The owner reported missing or unclear echo; the exact-call server record contains zero audio packets in either direction. See the [audio repair and build 18 evidence](IN-APP-ANSWER-AUDIO-20260911.md).
