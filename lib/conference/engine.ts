...
const requestKey = () =>
  `conference_${Date.now()}_${Math.random().toString(36).slice(2)}_${Math.random().toString(36).slice(2)}`;
/** Authenticated server control plane only. No ESL credentials or synthetic rooms on-device. */
export class ConferenceEngine {
  private client() {
    return createTRPCClient().conference;
  }
  async init(): Promise<void> {
    // Remove credentials and fabricated conference history left by the legacy demo.
    await AsyncStorage.multiRemove([
      "cloudphone11_freeswitch_esl",
      "cloudphone11_conferences",
    ]);
  }
  getCapabilities() {
    return this.client().capabilities.query();
  }
  getConference(id: string) {
    return this.client().detail.query({ conferenceId: id });
  }
  createConference(
    name: string,
  ...
  addParticipant(
    ...
    return this.client().addParticipant.mutate({
      conferenceId,
      extension,
      name,
      idempotencyKey,
  ...
  executeAction(
    ...
    return this.client().action.mutate({
      conferenceId,
      action,
      idempotencyKey,
    });
