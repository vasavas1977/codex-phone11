...
export const useConferenceStore = create<ConferenceStore>((set, get) => {
  async function run<T>(operation: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      set({
        error:
          error instanceof Error ? error.message : "Conference unavailable",
      });
      throw error;
    } finally {
      set({ isLoading: false });
  ...
  function apply(snapshot: Conference) {
    set((state) => ({
      activeConference: snapshot.state === "ended" ? null : snapshot,
      history: [snapshot, ...state.history.filter((c) => c.id !== snapshot.id)],
    }));
  }
  return {
    ...
    init: () =>
      run(async () => {
        ...
        const capability = await conferenceEngine.getCapabilities();
        if (!capability.audioAvailable) {
          set({
            activeConference: null,
            history: [],
        ...
        set({ history: await conferenceEngine.getConferenceHistory() });
      }),
    createConference: (name, config) =>
      run(async () => {
        const snapshot = await conferenceEngine.createConference(name, config);
        apply(snapshot);
        return snapshot;
      }),
    joinConference: (id) =>
      run(async () => {
        apply(await conferenceEngine.getConference(id));
      }),
    ...
    leaveConference: async () => {
      set({ activeConference: null });
    },
    addParticipant: (extension, name) =>
      run(async () => {
        const room = get().activeConference;
        if (room)
          apply(
            await conferenceEngine.addParticipant(room.id, extension, name),
          );
      }),
    executeAction: (action) =>
      run(async () => {
        const room = get().activeConference;
        if (room) apply(await conferenceEngine.executeAction(room.id, action));
      }),
    toggleRecording: async () => {
      const room = get().activeConference;
      if (room)
        await get().executeAction({
          type: room.isRecording ? "stop_recording" : "start_recording",
        });
    },
