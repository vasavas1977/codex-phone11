...
export type ChannelMeetingPickerProps = {
  ...
  loading?: boolean;
  /** A failed roster request must not be presented as an authorized empty roster. */
...
function Picker(props: ChannelMeetingPickerProps) {
  ...
  const rosterReady = !props.loading && !props.rosterError;
  const canStart = props.startAvailable && rosterReady && !props.busy && selectedIds.length > 0 && selectedIds.length <= (props.maxSelectedMembers ?? 50) && props.members.some(person => person.id === props.hostId);
  ...
  return <>
    <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "#00000055" }}>
      <View accessibilityViewIsModal style={{ maxHeight: "85%", backgroundColor: colors.background, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 34 }}>
        ...
        <TextInput accessibilityLabel="Search meeting participants" placeholder="Search members" placeholderTextColor={colors.muted} value={query} onChangeText={setQuery} editable={!props.busy && rosterReady} style={{ color: colors.foreground, backgroundColor: colors.surface, borderRadius: 12, padding: 12, minHeight: 44 }} />
        {rosterReady && invitees.length > 0 && <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginVertical: 8 }}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" style={{ flexGrow: 0 }}>
          ...
          {rosterReady && !matches.length && <Text style={{ paddingVertical: 20, color: colors.muted }}>No matching members</Text>}
        </ScrollView>
