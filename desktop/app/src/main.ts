...
async function bootstrap(): Promise<void> {
  ...
  window = new BrowserWindow({ width: 420, height: 740, minWidth: 360, minHeight: 600,
  ...
  ipcMain.handle(CHANNELS.signIn, checked(value => serial(() => handlers.signIn(value))));
  ipcMain.handle(CHANNELS.action, checked(value => handlers.action(value)));
  ipcMain.handle(CHANNELS.signOut, checked(() => serial(() => handlers.signOut())));
